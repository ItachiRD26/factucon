// Firmado de e-CF — implementación manual con node-forge + @xmldom/xmldom
// Sigue exactamente el ejemplo TypeScript oficial de DGII (Firmado_de_eCF.pdf)
// Algoritmos: RSA-SHA256 + C14N "compacta" (comportamiento real de DGII) + SHA256 digest + enveloped-signature
//
// Adaptado multi-tenant: el .p12 + contraseña se reciben como parámetros
// (cargados/desencriptados por el caller desde Firestore vía cert-crypto.ts),
// no desde variables de entorno como en la versión single-tenant original.
...
// p12Buffer: contenido binario del .p12 (ya desencriptado por cert-crypto.ts)
export function loadCertAndKey(p12Buffer: Buffer, password: string): CertData {
...
// ── Canonicalización C14N "compacta" (compatible con validador DGII) ───────────
// Verificada contra semilla firmada y e-CF certificados ante DGII:
...
function c14nNode(node: Node, inheritedNs: Record<string, string> = {}): string {
  ...
  for (let i = 0; i < el.childNodes.length; i++) {
    out += c14nNode(el.childNodes[i] as unknown as Node, nsNow);
  }
...
// ── Firmado principal ─────────────────────────────────────────────────────────
export async function firmarXML(xmlOriginal: string, cert: CertData): Promise<string> {
  const { privateKey, certBase64 } = cert;
...
export async function firmarSemilla(semillaXml: string, cert: CertData): Promise<string> {
  return firmarXML(semillaXml, cert);
}
