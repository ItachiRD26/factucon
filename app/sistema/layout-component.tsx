...
const NAV_ITEMS: Record<string, { icon: string; label: string; href: string }[]> = {
  owner:   [
    { icon: '📊', label: 'Dashboard', href: 'dashboard' },
    { icon: '🛒', label: 'POS',       href: 'pos' },
    { icon: '📦', label: 'Inventario',href: 'inventario' },
    { icon: '📋', label: 'Cotizaciones', href: 'cotizaciones' },
...
export function SistemaLayout({ children }: { children: React.ReactNode }) {
  ...
  const navItems = (NAV_ITEMS[session.role] ?? NAV_ITEMS.cashier)
    .filter(item => {
      // Solo mostrar módulos activos
      const moduleMap: Record<string, string> = {
        pos: 'pos', inventario: 'inventory', cotizaciones: 'quotes',
        clientes: 'clients', reportes: 'reports', facturas: 'ecf',
        dashboard: 'pos', // dashboard siempre visible
      };
      const moduleId = moduleMap[item.href];
      return item.href === 'dashboard' || session.modules.includes(moduleId);
    });
      ...
      <aside style={{ width: 220, flexShrink: 0, background: '#060D1F', borderRight: '1px solid rgba(255,255,255,.07)', display: 'flex', flexDirection: 'column', height: '100vh', position: 'sticky', top: 0 }}>
        ...
        <nav style={{ flex: 1, padding: '10px 8px', overflowY: 'auto' }}>
          {navItems.map(item => {
            ...
            const active  = pathname === href || pathname.endsWith(`/${item.href}`);
            return (
              <Link key={item.href} href={href} style={{
                display: 'flex', alignItems: 'center', gap: 10,
                padding: '8px 10px', borderRadius: 10, textDecoration: 'none',
                marginBottom: 2, fontSize: '0.82rem', fontWeight: active ? 600 : 500,
        ...
        <div style={{ padding: '10px 12px', borderTop: '1px solid rgba(255,255,255,.07)', background: 'rgba(0,0,0,.1)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ fontSize: '0.62rem', color: 'rgba(255,255,255,.35)' }}>{session.role}</div>
            </div>
          </div>
