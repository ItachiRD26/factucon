...
export async function GET(req: NextRequest) {
  ...
  const clients = snap.docs.map(d => {
    const data = d.data();
    return { id: d.id, name: data.name, rnc: data.rnc ?? '', phone: data.phone ?? '', email: data.email ?? '', address: data.address ?? '', creditLimit: data.creditLimit ?? 0, balance: data.balance ?? 0, isActive: data.isActive ?? true };
  });
...
export async function POST(req: NextRequest) {
  ...
  const { companyId, name, rnc, phone, email, address, creditLimit } = body;
  ...
  await ref.set({ id: ref.id, companyId, name: name.trim(), rnc: rnc ?? '', phone: phone ?? '', email: email ?? '', address: address ?? '', creditLimit: creditLimit ?? 0, balance: 0, isActive: true, createdAt: now, updatedAt: now });
